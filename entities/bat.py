...
from constant import BAT_HEALTH, BAT_ACTION_POINT, BAT_DAMAGES, BAT_COST
import pygame
import heapq
...
class Bat(Entity):
    ...
    """
    def __init__(self: Bat, image: str,  location: list[int, int]= [0,0], max_health: int = BAT_HEALTH, damage: int = BAT_DAMAGES, action_point: int = BAT_ACTION_POINT):
        """
        Initialization of the Bat entity
        Args:
            image (str): The image path of the bat
            ...
            action_point (int): The action point of the bat
        """
        super().__init__(image, location, max_health, damage, action_point)
        self._costs = BAT_COST 
        
    
    @property
    def costs(self: Bat) -> int:
        return self._costs
    @costs.setter
    def costs(self: Bat, cost: int):
        self._costs = cost
    
    def getAllMovements(self: Bat, players:list[Entity], grid: list[Case], nb_row: int, nb_col: int) -> list:
    ...
    def findClosestPlayer(self: Bat, grid: list[object], players: list[Entity], nb_row: int, nb_col: int) -> list[list[int, int]]:
        """
    ...
    def move(self: Bat, target: list[int, int], group: pygame.sprite.Group) -> pygame.sprite.Group:
        """
    ...
    def attack(self: Bat, target: Entity, grid: list[Case], turn_list: list[Entity], group: pygame.sprite.Group):
        """
