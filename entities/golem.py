...
from constant import GOLEM_HEALTH, GOLEM_DAMAGES, GOLEM_ACTION_POINT, GOLEM_COST, BROKEN_WALL
import pygame
import heapq

class Golem(Entity):
    """
    The class that represent the Golem entity that the players have to fight against
    """
    def __init__(self: Golem, image: str,  location: list[int, int]= [0,0], max_health: int = GOLEM_HEALTH, damage: int = GOLEM_DAMAGES, action_point: int = GOLEM_ACTION_POINT):
        """
        Initialization of the Golem entity
        Args:
            image (str): The image path of the golem
            ...
            action_point (int): The action point of the golem
        """
        super().__init__(image, location, max_health, damage, action_point)
        self._costs = GOLEM_COST # The number of coins the player have if he kill a golem
        
        
    @property
    def costs(self: Golem) -> int:
        return self._costs
    @costs.setter
    def costs(self: Golem, cost: int):
        self._costs = cost

    def getAllMovements(self: Golem, players: list[Entity], grid: list[Case], nb_row: int, nb_col: int) -> list:
        """ Get all the possible movements for the player

        Args:
            self (Golem): The player
        ...
        def explore_moves(x, y, depth_left):
            ...
            for dx, dy in directions:
                ...
                if 0 <= new_x < nb_row and 0 <= new_y < nb_col and not isinstance(grid[new_x][new_y], Lever):
    ...
    def findClosestPlayer(self: Golem, grid: list[object], players: list[Entity], nb_row: int, nb_col: int) -> list[list[int, int]]:
        """
        ...
        def is_valid_move(row, col):
            if 0 <= row < nb_row and 0 <= col < nb_col:
                cell = grid[row][col]
                return (not isinstance(cell, Lever))
            return False
    ...
    def move(self: Golem, target: list[int, int], grid: list[Case], group: pygame.sprite.Group) -> pygame.sprite.Group:
        """
        ...
        if isinstance(grid[self.location[0]][self.location[1]], Wall):
            grid[self.location[0]][self.location[1]].canCross = True
            grid[self.location[0]][self.location[1]].tile = BROKEN_WALL
        return group

    def attack(self: Golem, target: Entity, grid: list[Case], turn_list: list[Entity], group: pygame.sprite.Group):
        """
